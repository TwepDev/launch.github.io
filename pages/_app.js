...
function MyApp(p) {
  ...
  async function isSigned() {
    ...
    const x = await axios.request({
      method: 'POST', url: 'https://launch.twepv2.com/api/welaunch', data: { req: 'validate', address: address }, headers: {
    ...
    if (x.data.response == 200) {
      if (cookieCutter.get('welaunchSignMessage')) {
        ...
        const web3 = thisWeb3
        axios.request({ method: 'POST', url: 'https://launch.twepv2.com/api/welaunch', data: { req: 'sign_request', address: address } }).then(async function (response) {
          const message = response.data.data.message
          const mes = await web3.eth.personal.sign(message, address)
          axios.request({ method: 'POST', url: 'https://launch.twepv2.com/api/welaunch', data: { req: 'sign_done', message: message, signed: mes, address: address } }).then(async function (response) {
  ...
  async function sendData(req, form) {
    ...
    if (form) {
      ...
      const thisData = await axios.request({
        method: 'POST',
        url: 'https://launch.twepv2.com/api/welaunch',
      ...
      if (bearer) {
        const thisData = await axios.request({
          method: 'POST', url: 'https://launch.twepv2.com/api/welaunch', data: req, headers: {
            Authorization: bearer
          }
        })
        return thisData.data.data
      } else {
        const thisData = await axios.request({ method: 'POST', url: 'https://launch.twepv2.com/api/welaunch', data: req })
        return thisData.data.data
      }
    }
  }
  ...
  async function init() {
    ...
    const trends = await axios.request({ method: 'GET', url: 'https://launch.twepv2.com/api/trending/BSC-trend.json' })
